...
export interface RouterProviderProps {
   ...
   */
  routes?: Record<string, () => Promise<{ default: ComponentType<any>, meta?: any }>>;
  
  /**
   * Map of layout patterns to dynamic imports
   */
  layouts?: Record<string, () => Promise<{ default: ComponentType<{children: ReactNode}>, meta?: any }>>;
}
...
function RecursiveLayouts({ 
  ...
  layouts: Record<string, { Component: ComponentType<{children: ReactNode}>, meta?: any }>, 
  pathname: string, 
  children: ReactNode 
}) {
    ...
    const LayoutComponents = activeLayoutPaths
        ...
        .filter((l): l is { Component: ComponentType<{children: ReactNode}>, meta?: any } => !!l && !!l.Component)
        .map(l => l.Component);
    ...
    return LayoutComponents.reduceRight((acc, Layout) => {
...
export function BuncfRouter({
  ...
  const [loadedLayouts, setLoadedLayouts] = useState<Record<string, { Component: ComponentType<any>, meta?: any }>>({});
  ...
  const applyMeta = (metaFns: any[], data: any, params: any) => {
      try {
          ...
          document.querySelectorAll('meta[data-buncf="true"]').forEach(el => el.remove());
  ...
  async function loadPage(pathname: string) {
    ...
    try {
      ...
      const segments = pathname.split("/").filter(Boolean);
      const pathsToCheck = ["/", ...segments.map((_, i) => "/" + segments.slice(0, i + 1).join("/"))];
      
      const newLayouts: Record<string, { Component: ComponentType<any>, meta?: any }> = { ...loadedLayouts };
      let layoutsUpdated = false;

      // Load all applicable layouts in parallel
      await Promise.all(pathsToCheck.map(async (pathKey) => {
          ...
          if (importer) {
              try {
                  ...
                  if (mod.default) {
                      newLayouts[pathKey] = { Component: mod.default, meta: mod.meta };
                      layoutsUpdated = true;
                  }
              } catch (e) {
      ...
      // 4. Run Loader if exists
      // @ts-ignore
      if (module.loader && typeof module.loader === "function") {
          try {
              // @ts-ignore
              const data = await module.loader({ params: matchedParams, query: routerStore.getState().query });
              if (routerStore.getState().pathname !== pathname) return;
              setPageData(data);
          } catch (loaderErr) {
              if (routerStore.getState().pathname !== pathname) return;
      ...
      const layoutPaths = ["/", ...segments.map((_, i) => "/" + segments.slice(0, i + 1).join("/"))];
  ...
  const contentNode = (
      <>
        {PageComponent ? <PageComponent params={route.params} query={route.query} data={pageData} /> : <div />}
      </>
  );
  ...
  // Error Handling
  if (error) {
    const CustomError = routes["/_error"] ? React.lazy(routes["/_error"] as any) : null;
    ...
    return LegacyRootLayout ? <LegacyRootLayout>{ErrorComp}</LegacyRootLayout> : ErrorComp;
  }

  // Loading Handling
  const CustomLoading = routes["/_loading"] ? React.lazy(routes["/_loading"] as any) : null;
  if (loading && CustomLoading) {
      ...
      return LegacyRootLayout ? <LegacyRootLayout>{LoadingComp}</LegacyRootLayout> : LoadingComp;
  }
