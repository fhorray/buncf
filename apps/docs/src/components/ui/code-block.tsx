...
export function CodeBlock({
  ...
  return (
    <div
      className={cn(
        'my-6 relative overflow-hidden rounded-xl border border-white/10 bg-[#0e0e0e] shadow-2xl',
        className,
      )}
    >
      {/* Header */}
            ...
            <pre style={{ ...style, background: 'transparent' }}>
              {tokens.map((line, i) => (
                <div key={i} {...getLineProps({ line })}>
                  <span className="inline-block w-8 select-none text-white/20 text-xs text-right mr-4">
                    {i + 1}
                  </span>
                  {line.map((token, key) => (
